...
module {
  ...
  public class CourseService() {
    private var courses = HashMap.HashMap<Text, Course>(10, Text.equal, Text.hash);
    private var coursesByCategory = HashMap.HashMap<Text, [Text]>(10, Text.equal, Text.hash);
    private var coursesByLevel = HashMap.HashMap<Text, [Text]>(10, Text.equal, Text.hash);
    ...
    public func importCources(storage: CourseStorage) {
      courses := HashMap.fromIter<Text, Course>(storage.courses.vals(), storage.courses.size(), Text.equal, Text.hash);
      coursesByCategory := HashMap.fromIter<Text, [Text]>(storage.coursesByCategory.vals(),
        storage.coursesByCategory.size(), Text.equal, Text.hash);
      coursesByCategory := HashMap.fromIter<Text, [Text]>(storage.coursesByLevel.vals(),
        storage.coursesByLevel.size(), Text.equal, Text.hash);
    };
    ...
    public func getCoursesByCategories(categories: [Text]): Types.Response<[Course]> {
      ...
      for (category in Iter.fromArray(categories)) {
        Option.iterate(coursesByCategory.get(category), func(ids: [Text]) {
      ...
      let ids: [Text] = ArrayUtils.removeArrayDuplicates(courseIds.toArray(), Text.hash, Text.equal);
    ...
    public func updateCourse(request: UpdateCourseRequest): Types.Response<Course> {
      ...
      switch (getCourse(request.id)) {
        case (#ok(course)) {
          ...
          updateCoursesByCategory(updatedCourse.id, course.categories, updatedCourse.categories);
          updateCoursesByLevel(updatedCourse.id, ?course.level, ?updatedCourse.level);
    ...
    private func updateCoursesByCategory(courseId: Text, oldCategories: [Text], newCategories: [Text]) {
      HashMapUtils.updateHashMapOfArrays(coursesByCategory, courseId, oldCategories, newCategories,
        Text.equal, Text.equal, Text.hash);
    };

    private func updateCoursesByLevel(courseId: Text, oldLevel: ?CourseLevel, newLevel: ?CourseLevel) {
      ...
      HashMapUtils.updateHashMapOfArrays(coursesByLevel, courseId, oldKeys, newKeys, Text.equal, Text.equal, Text.hash);
    };
