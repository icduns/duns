...
module {
  ...
  public func getFromHashMap<K, V>(map: HashMap.HashMap<K, V>, keys: [K]): [V] {
    ...
    for (key in Iter.fromArray(keys)) {
      Option.iterate(map.get(key), func(value: V) {
        values.add(value);
      });
    };
  ...
  public func updateHashMap<K,V>(map: HashMap.HashMap<K, V>, value: V, oldKeys: [K], newKeys: [K]) {
  ...
  public func updateHashMapOfArrays<K, V>(map: HashMap.HashMap<K, [V]>, value: V, oldKeys: [K], newKeys: [K],
    valueEq: (V, V) -> Bool, keyEq: (K, K) -> Bool, keyHash: K -> Hash.Hash) {
    ...
    for (key in removeFrom.vals()) {
      Option.iterate(map.get(key), func(values: [V]) {
        let newValues: [V] = ArrayUtils.removeFromArray<V>(values, value, valueEq);
        if (newValues.size() == 0) {
          map.delete(key);
        } else {
